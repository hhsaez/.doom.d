...
;; native indentation while editing, LSP formatting on save when available.
(after! org-src
...
Ensure regular C++ buffers behave the same way: native indentation while editing,
and LSP formatting on save when available.
...
(after! cc-mode
  ...
  ;; Main entry point: set native indentation, hook formatting on save,
  ;; and keep TAB on the native indentation path.
