...
Ensure regular C++ buffers use native indentation while editing. Save-time
formatting is handled by Doom's =format +onsave +lsp= module.

#+BEGIN_SRC emacs-lisp
;; Keep C++ editing fast and predictable: native indentation while editing.
(after! cc-mode
  ;; Tree-sitter C++ mode uses a different indentation engine, so keep classic
  ;; cc-mode as the default when native indentation fidelity matters.
