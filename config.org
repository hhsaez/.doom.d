...
;; Keep inline Org src blocks simple; use `C-c '` for real language indentation.
(after! org
  ...
  (defun hhsaez/org-inline-src-indent-setup ()
    (setq-local indent-tabs-mode nil
                tab-width 3))

  (defun hhsaez/org-inline-src-tab-h ()
  ...
  (add-hook 'org-mode-hook #'hhsaez/org-inline-src-indent-setup)
  (add-hook 'org-tab-first-hook #'hhsaez/org-inline-src-tab-h)
...
(after! org-src
  (defun hhsaez/org-src-cpp-lsp-format-setup ()
  ...
  (add-hook 'org-src-mode-hook #'hhsaez/org-src-cpp-lsp-format-setup))
#+END_SRC
...
(after! cc-mode
  (defconst hhsaez/c++-fallback-style
    '("stroustrup"
      (c-basic-offset . 3)
      ...
      (c-offsets-alist
       ...
       (topmost-intro-cont . +)
       (template-args-cont . +)))
    "Fallback C++ indentation style that approximates the preferred ClangFormat.")

  (c-add-style "hhsaez-clang-format" hhsaez/c++-fallback-style)

  (defun hhsaez/lsp-formatting-available-p ()
  ...
  (defun hhsaez/c++-apply-fallback-style ()
    (setq-local indent-tabs-mode nil
                tab-width 8)
    (when (derived-mode-p 'c++-mode)
      (c-set-style "hhsaez-clang-format")
      (setq-local c-basic-offset 3))
    (when (boundp 'c-ts-mode-indent-offset)
      (setq-local c-ts-mode-indent-offset 3)))

  ;; Pull `.clang-format` width overrides into the local indentation settings.
  (defun hhsaez/c++-apply-clang-format-overrides ()
  ...
  (defun hhsaez/c++-lsp-format-setup ()
    (hhsaez/c++-apply-fallback-style)
    (hhsaez/c++-apply-clang-format-overrides)
    (add-hook 'before-save-hook #'hhsaez/lsp-format-buffer-if-ready nil t)
    (local-set-key (kbd "TAB") #'hhsaez/lsp-format-or-indent)
    (local-set-key (kbd "<backtab>") #'indent-for-tab-command))
