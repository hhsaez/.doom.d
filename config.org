...
after two characters; use three instead, and do not auto-popup after C++ access
labels such as =public:=.
...
(after! corfu-auto
  ...
  (defun hhsaez/corfu-auto-complete-unless-c++-access-label (fn &rest args)
    "Prevent delayed Corfu completion after C++ access labels."
    (unless (hhsaez/c++-access-label-colon-p)
      (apply fn args)))

  (advice-add #'corfu-auto--complete-deferred
              :around #'hhsaez/corfu-auto-complete-unless-c++-access-label)
...
Ensures Emacs’ default next-error selection order: latest compilation buffer first, current Flymake-capable buffer only as fallback
(overrides DOOM Emacs behavior to use the current source buffer)
