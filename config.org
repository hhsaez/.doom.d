    ...
    (dolist (line banner)
      (insert
       (+doom-dashboard--center
        ...
        (concat line
                (make-string (max 0 (- longest-line (string-width line))) ?\s))))
      (insert "\n"))
    (add-text-properties start (point) '(face doom-dashboard-banner))))

(defun my/doom-dashboard-banner-from-file ()
  (let* ((banner-file (expand-file-name "banner.txt" doom-user-dir))
         (banner (with-temp-buffer
                   (insert-file-contents banner-file)
                   (split-string (buffer-string) "\n" t)))
...
(setq +doom-dashboard-ascii-banner-fn #'my/doom-dashboard-banner-from-file)
...
Install missing grammars automatically so tree-sitter works on fresh setups.

#+BEGIN_SRC emacs-lisp
(after! treesit
  (require 'cl-lib)
  (defvar hhsaez/treesit-auto-grammars '(c cpp css javascript json tsx typescript yaml)
